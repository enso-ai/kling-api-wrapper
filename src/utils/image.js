 ...
 */
export const calculateOptimalDimensions = (originalWidth, originalHeight, constraints = IMAGE_CONSTRAINTS) => {
  const { MIN_DIMENSION, MAX_DIMENSION } = constraints;
  
  // Check if already within bounds
  if (originalWidth >= MIN_DIMENSION && originalWidth <= MAX_DIMENSION &&
      originalHeight >= MIN_DIMENSION && originalHeight <= MAX_DIMENSION) {
    return {
      width: originalWidth,
  ...
  if (newWidth < MIN_DIMENSION || newWidth > MAX_DIMENSION ||
    ...
    if (aspectRatio > maxAspectRatio) {
      return {
        ...
        height: originalHeight,
        needsResize: false,
        error: `Image is too wide. Maximum supported aspect ratio is ${maxAspectRatio.toFixed(1)}:1`
      };
      ...
      return {
        ...
        height: originalHeight,
        needsResize: false,
        error: `Image is too tall. Minimum supported aspect ratio is 1:${(1/minAspectRatio).toFixed(1)}`
      };
    }
  }
 ...
 */
export const resizeImageWithQualityAdjustment = (file, targetWidth, targetHeight, maxFileSize = IMAGE_CONSTRAINTS.MAX_FILE_SIZE_BYTES) => {
  return new Promise((resolve, reject) => {
    const img = new Image();
    const canvas = document.createElement('canvas');
    const ctx = canvas.getContext('2d');
    ...
    img.onload = async () => {
      ...
      URL.revokeObjectURL(img.src);
      const fileSizeMB = (estimatePngSize(targetWidth, targetHeight, 0.4) / (1024 * 1024)).toFixed(1);
      reject(new Error(`Resized image would be too large (estimated ${fileSizeMB} MB). Try a smaller source image.`));
    };
